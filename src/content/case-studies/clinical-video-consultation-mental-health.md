...
quickRead: |
  **The distinctive decision in this engagement was a refusal.** The EU AI Act (Article 5(1)(f)) prohibits emotion recognition in workplace and education contexts but exempts medical purposes. We chose not to use the medical exemption — anchored on Lisa Feldman Barrett's 2019 meta-analysis of 1,000+ studies showing facial expression does not reliably infer emotion. The platform measures *movement*, not emotion. Zero emotion labels, zero confidence scores, zero temporal-emotion graphs. The system never tells a clinician what a patient is feeling.

  The brief: a self-hosted, EU-only video-consultation platform that delivers measurably better signal for the channels clinicians actually use — facial micro-expressions and vocal tonality — than Zoom or Teams can at the same bandwidth, handles face-mesh data as the GDPR Article 9 biometric data it is, and stays clear of the AI Act's high-risk gate.

  The organising principle is **"client works, server forwards"** — face-mesh extraction, neural noise cancellation, ROI encoding, and adaptive framerate all run on the client device; the server is a smart switch. Server CPU stays under 15% at full capacity. The marginal cost of higher quality is zero. The platform delivers a ~2.5× sharper face region at the same total bitrate, 96 kbps Opus audio (3× Zoom's default), and a cumulative **98% storage reduction** at the cold tier across the full codec, composite, face-mesh, and retention pipeline. €2,190/month down to €45–55/month at 500 sessions/day. Per-session AES-256-GCM keys from HashiCorp Vault, crypto-shredding for GDPR Article 9 right-to-erasure in under 24 hours — audited at the level a notified body could verify.
