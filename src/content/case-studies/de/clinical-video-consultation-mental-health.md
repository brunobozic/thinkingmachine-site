...
quickRead: |
  Ein EU-Anbieter im Bereich psychischer Gesundheit benötigte eine selbst-gehostete, EU-only Video-Konsultations-Plattform, die (a) messbar besseres Signal für die Kanäle liefert, die Kliniker tatsächlich nutzen — Mikro-Ausdrücke im Gesicht und stimmliche Tonalität — als Zoom oder Teams bei gleicher Bandbreite, (b) Face-Mesh-Daten als die biometrischen Daten nach DSGVO Artikel 9 behandelt, die sie sind, und (c) sich von der Hochrisiko-Klassifikation des EU AI Act für Emotionserkennungs-Systeme fernhält.

  Das organisierende Prinzip der Architektur lautet **"Der Client arbeitet, der Server leitet weiter"** — alle ML (Face-Mesh-Extraktion, Geräuschunterdrückung, ROI-Encoding, adaptive Framerate) läuft auf dem Client-Gerät; der Server ist ein intelligenter Switch. Ergebnis: Server-CPU bleibt selbst bei voller Kapazität unter 15%, die Grenzkosten höherer Qualität sind null, und die Plattform liefert ~2,5× schärferes Gesicht bei gleicher Gesamt-Bitrate, 96 kbps Opus-Audio (3× Zooms Default), und ~50% Speicher-Reduktion via Composite Recording — kumulativ **98% Speicher-Reduktion auf der Cold-Tier** über die volle Codec-+-Composite-+-Face-Mesh-+-Retention-Pipeline. €2.190/Monat hinunter auf €45–55/Monat bei 500 Sitzungen/Tag.

  **Die unterscheidende architektonische Entscheidung: bewusst *keine* Emotionserkennung.** Der EU AI Act (Artikel 5(1)(f)) verbietet Emotionserkennung in Arbeits- und Bildungskontexten, erlaubt sie aber medizinisch über eine enge Ausnahme. Wir wählten, die medizinische Ausnahme nicht zu nutzen, gestützt auf Lisa Feldman Barretts Meta-Analyse von 2019 (über 1.000 Studien), die zeigt, dass Gesichtsausdrücke keine zuverlässige Emotionsinferenz erlauben. Die Plattform misst *Bewegung*, nicht Emotion. Null Emotions-Labels, null Konfidenz-Werte, null zeitliche Emotions-Graphen. Das System sagt einem Kliniker nie, was ein Patient fühlt.

  Geliefert als Architektur-Spezifikation und Technologie-Auswahl — bereit für gestaffelte Implementierung, gerechtfertigt auf einem Niveau, das ein Regulierer oder eine Benannte Stelle auditieren könnte.
