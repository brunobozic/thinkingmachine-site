...
quickRead: |
  Multi-tenant SaaS operator na Hetzner Cloudu trebao je tri stvari odjednom: **pet godina star Ansible/Semaphore IaC stack oživljen end-to-end** (izvorni inženjeri su otišli, platforma više nije radila čisto), **interni sloj softverskog opskrbnog lanca** (PHP s javnog Packagista, JavaScript s javnog npm-a, SQL artefakti s ad-hoc developerskih strojeva — bez internog korijena povjerenja, bez evaluacijskih vrata prije integracije), i **analitički stack** (Metabase + Grafana na vlastitim VM-ovima, povezano na isti PKI kao ostatak platforme).

  Kroz **četiri međusobno povezana repozitorija i ~178 commitova**, mandat je rekonstruirao orkestracijski / IaC repozitorij (~85 commitova — integracija Semaphore API-ja, higijena SSH ključeva, tok docker-login-a, lifecycle certifikata), oživio **produkcijski nginx reverse-proxy** (~67 commitova, označeno za produkciju kroz više objavljenih verzija — WebSocket podrška za Grafanu, po-servis auth usmjeravanje, prilagođene stranice grešaka), radio na **PostgreSQL container base imageu** (~20 commitova — Postgresovi logovi pouzdano prelaze granice kontejnera), i isporučio **feature branch analitičkog stacka** (~6 commitova) koji je doveo Metabase i Grafanu u isti Ansible/Semaphore put provizioniranja.

  Razlikovni element — otprilike 30% mandata — bila su **četverostepena vrata prije integracije** za svaki kanal artefakata: developer traži paket → statička analiza (composition scan, inventar licenci, signali na razini koda) → skeniranje ranjivosti prema CVE / GHSA / OSV → odobrenje i ogledavanje u internom registru (**Satis za Composer**, **Verdaccio za npm**, **Redgate SQL Source Control** za promjene sheme, **GitLab Container Registry** za slike). Ne postoji resolver put koji zaobilazi interni registar.

  Obrazac se izravno preslikava na **EN/IEC 62304 § 5.1.5 + § 8.1 (identifikacija SOUP + pregled anomalija), MDCG 2019-16, IEC 81001-5-1, NIS2 članak 21(2)(d) i CRA članak 13 + Aneks I dio II(1)/(2)** — kontrole opskrbnog lanca koje regulativa sada zahtijeva od proizvođača. Implementirano 2024., dvije godine prije nego što CRA Cliff 2 (prosinac 2027.) od njih napravi pune obveze.
