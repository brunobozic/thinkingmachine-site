...
Početni potez bio je natjerati postojeći stack da ponovno radi end-to-end iz rekonstrukcije. Bez greenfield prepisivanja. Čitanje postojećeg Ansible koda, strukture inventara (`group_vars`, `host_vars`, `playbooks`, `tasks`, `roles`, `templates`), definicija Semaphore predložaka i toka docker-login-a prema GitLab Container Registryju, a zatim vježbanje svakog puta stvarnim provizioniranjima prema Hetzneru.

Ta faza proizvela je otprilike 85 commitova kroz IaC repozitorij — vidljivih u javno dostupnim dijelovima traga kao male, inkrementalne, debugging-razinske poruke (tipičan potpis rada protiv klimavog vanjskog API-ja i lanca provizioniranja više VM-ova). Klasteri su bili:

- **Integracija Semaphore API-ja.** Idempotentno stvaranje korisnika, opseg varijabli predložaka, neslaganja oblika JSON objekt-vs-niz (Semaphore HTTP API očekuje nizove gdje je templating playbooka proizvodio objekte), debug 400-pri-stvaranju-korisnika. Rezultat je Semaphore projekt koji administratori mogu koristiti za samouslužno stvaranje novih developerskih VM-ova i klijentskih okolina bez izravnog diranja u Ansible.
- **Higijena SSH ključeva.** Developerski SSH ključevi tiho su se propagirali na svaki provisionirani VM. Popravak uklanja taj put i dokumentira zašto je promjena strukturna, a ne kozmetička — mali dokaz da se pozicija opskrbnog lanca proteže i na granicu vjerodajnica između razvoja i proizvodnje.
- **docker-login prema registryju.** Dva odvojena GitLab login toka (jedan za registry, jedan za API) su se brkali. Popravak ih razdvaja, izlaže vjerodajnice kroz Makefile cilj i daje playbooku provjerljiv login korak prije bilo kakvog povlačenja slike.
- **Lifecycle certifikata.** Sredstva PKI / CA (wildcard certifikat za internu TLD, po-VM developerski certifikati, lanac povjerenja internog CA) čisto integrirana s putevima provizioniranja dev-VM-a i analytics-VM-a, tako da svaki čvor govori mTLS iz jedinstvenog internog korijena povjerenja.
...
Dodana su dva dodatna Hetzner VM-a — jedan za Metabase, jedan za Grafanu — svaki proviziran kroz isti Ansible/Semaphore put koji se koristi za developerske i klijentske VM-ove. Metabase VM hosta BI alat s vlastitim PostgreSQL skladištem metapodataka i Java Keystoreom za TLS terminaciju. Grafana VM koristi isti interni CA, persistira stanje pod `/grafana` i pokreće `grafana/grafana-oss` sliku u Dockeru iza iste konvencije imenovanja DNS zone kao ostatak platforme. Oba čvora dostupna su samo unutar OpenVPN sloja; nijedan nije izložen javnom internetu.

## Ishod

Platformska IaC ponovno radi end-to-end, s pokretnim dijelovima dokumentiranima (arhitektura, playbooki, uloge, workflowi) na razini koja preživi sljedeći događaj rotacije inženjera. Sloj opskrbnog lanca znači da je odgovor na pitanje *Možete li opisati kako bi typosquattirani javni paket dosegnuo proizvodnju* — "Ne može, jer su kanali artefakata posredovani internim registrijima." Analitički stack radi uz ostatak platforme bez proširenja javne napadne površine.
...
Single-inženjer intenzivno kroz nekoliko mjeseci, radeći uz mali interni tim operatora. Otprilike 85 commitova na IaC repozitoriju kao vidljiv trag, plus provizioniranje analitičkog stacka i četiri komponente registrija opskrbnog lanca. Cijelo vrijeme povjerljivo; ova stranica je jedina anonimizirana referenca.
