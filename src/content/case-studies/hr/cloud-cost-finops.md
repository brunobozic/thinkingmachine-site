...
quickRead: |
  **Najvrjedniji redak u isporuci bio je popravak SQL konfiguracije bez troška.** U testnom okruženju postavka paralelizma stvarala je prividnu potrebu za nadogradnjom razreda baze podataka. Označili smo je kao prvu stavku za akciju, ne sakrili u dodatku — i ona iznova oblikuje cijeli razgovor o dimenzioniranju prije nego što se odobri ijedan euro nove infrastrukture.

  Industrijski softverski dobavljač s više instalacija pripremao je ponudu za Cloud-managed SaaS posao tier-1 korporativnom kupcu. Komercijalno pitanje djelovalo je varljivo jednostavno: *koliko naplatiti po instalaciji mjesečno?* Bazna linija postojećeg hostinga bila je neprozirna (managed-services partner odbio je predati račune). Dimenzioniranje opterećenja bilo je neizvjesno (raspoloživo je bilo samo umanjeno testno okruženje). Posao je nosio i ne-troškovne dimenzije koje su morale stajati uz brojku troška, a ne iza nje.

  U četrdeset sati kroz oko četiri tjedna isporučili smo paket spreman za odluku na razini uprave: triangulirana bazna linija sastavljena od provjerenih cijena s cjenika javnog clouda × tipičnog raspona partnerske marže — tehnika koja omogućuje obranjivu brojku troška kad računi nisu dostupni — matrica scenarija na tri razine obveze, playbook cijena na tri razine marže i dvije razine kadra, kalkulator na strani kupca koji je mogao sam popunjavati, te NFR scorecard prema kupčevoj nabavnoj matrici.

  Dobavljač je na sljedeći sastanak s kupcem ušao s obranjivim modelom cijene po instalaciji i čistim razdvajanjem komercijalne cijene od troška infrastrukture. Pitanje *postoji li uopće marža?* — prije neodgovorivo — preokrenuto je u razgovor o SaaS premiji potkrijepljenu kvantificiranom baznom linijom.
