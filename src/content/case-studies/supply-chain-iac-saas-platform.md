...
quickRead: |
  A multi-tenant SaaS operator on Hetzner Cloud needed three things at once: a **five-year-old Ansible/Semaphore IaC stack revived end-to-end** (the original engineers had moved on, the platform no longer ran cleanly), an **internal software supply-chain layer** (PHP from public Packagist, JavaScript from public npm, SQL artifacts from ad-hoc developer machines — no internal trust root, no pre-integration evaluation gate), and an **analytics stack** (Metabase + Grafana on their own VMs, wired into the same PKI as the rest of the platform).

  Across **four interconnected repositories and ~178 commits**, the engagement reconstructed the orchestration / IaC repository (~85 commits — Semaphore API integration, SSH-key hygiene, docker-login flow, certificate lifecycle), brought the **production nginx reverse-proxy** back to life (~67 commits, production-tagged across multiple released versions — WebSocket support for Grafana, per-service auth routing, custom error pages), worked the **PostgreSQL container base image** (~20 commits — Postgres logs crossing container boundaries reliably), and shipped the **analytics-stack feature branch** (~6 commits) that landed Metabase and Grafana into the same Ansible/Semaphore provisioning path.

  The distinctive piece — roughly 30% of the engagement — was the **four-step pre-integration gate** for every artifact channel: developer requests a package → static analysis (composition scan, license inventory, code-level signal) → vulnerability scan against CVE / GHSA / OSV → approval and mirror into the internal registry (**Satis for Composer**, **Verdaccio for npm**, **Redgate SQL Source Control** for schema changes, **GitLab Container Registry** for images). No resolver path bypasses the internal registry.

  The pattern maps directly onto **EN/IEC 62304 § 5.1.5 + § 8.1 (SOUP identification + anomaly review), MDCG 2019-16, IEC 81001-5-1, NIS2 Article 21(2)(d), and CRA Article 13 + Annex I Part II(1)/(2)** — the supply-chain controls the regulation now requires manufacturers to evidence. Implemented in 2024, two years before CRA Cliff 2 (December 2027) makes them full obligations.
