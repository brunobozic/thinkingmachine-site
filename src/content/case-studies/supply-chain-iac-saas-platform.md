...
quickRead: |
  **Implemented in 2024 — two years before CRA Cliff 2 (December 2027) makes supply-chain integrity a regulatory obligation across the EU.** The pattern maps directly onto EN/IEC 62304 SOUP controls, NIS2 Article 21, and CRA Annex I — the supply-chain regime regulators now require manufacturers to evidence.

  A multi-tenant SaaS operator on Hetzner Cloud needed three things at once: a five-year-old Ansible/Semaphore IaC stack revived end-to-end, an internal software supply-chain layer (PHP from public Packagist, JavaScript from public npm, SQL artifacts from ad-hoc developer machines — no internal trust root, no pre-integration evaluation gate), and an analytics stack (Metabase + Grafana on their own VMs, wired into the same PKI as the rest of the platform).

  Across four interconnected repositories — orchestration / IaC, production nginx reverse proxy, PostgreSQL container, analytics-stack feature branch — approximately 178 commits restored the platform end-to-end.

  The distinctive piece — roughly 30% of the engagement — was the **four-step pre-integration gate** for every artifact channel: developer requests a package → static analysis (composition scan, license inventory, code-level signal) → vulnerability scan against CVE / GHSA / OSV → approval and mirror into the internal registry (Satis for Composer, Verdaccio for npm, Redgate SQL Source Control for schema changes, GitLab Container Registry for images).

  **No resolver path bypasses the internal registry.** That is the architectural assertion buyers and regulators remember.
